...
| --- | --- | --- |
| 3D explorer | `/explorer/` | 2,299 individually selectable pieces (1,736 named structures) in 19 systems, male and female, streamed as compressed glTF; search, organs, regions, isolate, x-ray, slicing on three planes, exploded and inventory views, pinned labels, deep links, clinical topics on every structure card, study modes, embeddable. |
| Anatomy | `/anatomy/<organ>/` | The canonical page of each of 48 organs and skeletal groups; 20 carry full articles (heart, brain, lungs, liver, kidneys, stomach, pancreas, aorta, coronary arteries, knee, spine, large intestine, uterus and cervix, ovaries and fallopian tubes, vagina, vulva and clitoris, breasts, pelvic floor, neovagina, neophallus): key facts, location, structure, supply, function, clinical relevance, references, a click-to-load 3D model and every topic that concerns the organ. The reference body is an adult male; the female pelvic organs, bladder and breasts are modelled by the Human Reference Atlas layer and the anatomy of gender-affirming surgery by two schematic layers, so only the vulva and clitoris still show the modelled structures around them in 3D instead of a model of their own. `/anatomy/` and `/organs/` are the browse hubs. |
| Body systems | `/systems/<system>/` | 19 system pages: overview, functions, clinical notes, organs, every structure, 3D model, related topics. The male reproductive system, the female reproductive layer (uterus, tubes, ovaries, vagina, ligaments, bladder, breasts) and the two schematic gender-affirming surgery layers (vaginoplasty, phalloplasty) each have a page. |
...
| Medical tests | `/tests/<test>/` | 39 tests (23 at full clinical depth): kind, category, specimen, method, LOINC codes, what they measure, why ordered, how done, reading the result, limitations. `/tests/categories/` is the master taxonomy: 36 categories over 837 catalogued test concepts. |
...
Database Center for Life Science, licensed CC BY 4.0, an adult male. The female pelvic organs and
breasts (system `reproductive-female`) come from the **NIH Human Reference Atlas** united female
model v1.5 (Visible Human Female, CC BY 4.0), fitted into the same frame by
`tools/build-female-atlas.mjs` from the mapping in `tools/manifest/female-source.json`; the
`vaginoplasty` and `phalloplasty` systems are schematic geometry generated by the same tool. The layers
are hidden by default in the explorer and switch on from the systems list (the *Female body* preset shows the
whole body with the female organs). See `data/ATTRIBUTION.md`.
...
# 3. add the female body and gender-affirming layers (needs the HRA united-female GLBs, e.g. public/anatomy/ of
#    https://github.com/Nurkan1/Anatria-3D); reads the HD and lite settings from the atlas.json files just built
node build-female-atlas.mjs --src /path/to/hra-female-glbs
# 4. compile content, 5. render the 3D preview images (needs a local server on :8123)
...
Database Center for Life Science (the male reference body) and the NIH Human Reference Atlas united
female model, built from the Visible Human Female (the female layer); the schematic gender-affirming
layers and the written descriptions in `content/` are this project's own work, CC BY 4.0. Full detail in
`data/ATTRIBUTION.md`. This is an
