...
import { renderHeader, renderFooter, loadData, loadClinical, link, esc, paths, PRERENDERED, SITE, breadcrumbHtml } from '../site/site.js?v=1.1.1';
import { relatedCountForAnatomy } from '../site/entity.js?v=1.1.1';
import { applyMeta, metaDescription, webPageNode } from '../site/seo.js?v=1.1.1';
...
if (!PRERENDERED) {
  ...
  document.getElementById('main').innerHTML = `
    ...
    <div class="section-hero"><div class="eyebrow">Browse</div><h1>Organs by body system</h1><p class="lead">${content.organs.length} organs and skeletal groups assembled from the pieces of the 3D atlas, grouped by the system they belong to. Each opens its anatomy page: structure, function, a live 3D model and every clinical topic that concerns it.</p></div>
    ${atlas.systems.filter(s => bySys.has(s.id)).map(s => `<h2><span class="dot" style="background:${s.color}"></span><a href="${link.systemPage(s.id)}">${esc(s.name)}</a></h2><div class="grid grid-3">${bySys.get(s.id).map(o => `<a class="card" href="${link.organPage(o.id)}"><h3>${esc(o.name)}</h3><p>${esc(o.summary)}</p><div class="meta">${o.structures.length} structures · ${relatedCountForAnatomy('organs', o.id, clinical)} topics${o.region ? ' · ' + esc(content.regions.find(r => r.id === o.region)?.name || '') : ''}</div></a>`).join('')}</div>`).join('')}
