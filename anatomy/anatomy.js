...
import { renderHeader, renderFooter, loadData, loadClinical, link, esc, pageId, paths, PRERENDERED, SITE, breadcrumbHtml, facadeHtml, canonical, entityLink, TYPE_ORDER, organHash } from '../site/site.js?v=1.8.0';
import { relatedForAnatomy, relatedCountForAnatomy, referencesHtml, editorialHtml } from '../site/entity.js?v=1.8.0';
import { applyMeta, seoTitle, metaDescription, webPageNode } from '../site/seo.js?v=1.8.0';
...
if (!PRERENDERED) {
  ...
  const modelled = structs.length > 0; const nearby = (o.nearby || []).map(i => atlas.structures[i]);
  const hash = organHash(o, atlas);           // o=<organ>, or s=<the structures around an unmodelled organ>, or null
  ...
  const description = art ? `Explore the anatomy of the ${o.name.toLowerCase()}: ${art.descriptor.toLowerCase()}, clinical relevance and ${modelled ? 'an interactive 3D model' : 'the surrounding structures in 3D'}.` : `${o.name}: ${o.summary}`;
  ...
  applyMeta({ title, description, path, robots: o.index ? 'index,follow' : 'noindex,follow', image: link.preview(hash || ''), breadcrumbs: crumbs,
  ...
  main.innerHTML = `
    ...
    <div class="actions">
      ${modelled ? `<a class="btn btn-primary" href="${link.organ(o.id)}">Open in the 3D explorer</a>
      ...
      : `${hash ? `<a class="btn btn-primary" href="${link.explorer(hash)}">Surrounding structures in 3D</a>` : ''}${region ? `<a class="btn" href="${link.region(region.id)}">Show the ${esc(region.name.toLowerCase())} in 3D</a>` : ''}`}
    </div>
    ${hash ? facadeHtml(hash, modelled ? o.name : `${o.name}: surrounding structures`) : ''}
    <div class="two">
      <div>
        ...
        ${modelled ? `<h2>Structures in the atlas <span class="badge">${structs.length}</span></h2>
