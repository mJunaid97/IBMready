...
import { renderHeader, renderFooter, loadData, link, esc, fmt } from '../site/site.js';
...
document.getElementById('systems').innerHTML = atlas.systems.map(s => {
  ...
  return `<a class="card" href="${link.systemPage(s.id)}"><h3><span class="dot" style="background:${s.color}"></span>${esc(s.name)}</h3><p>${esc(c.summary || s.summary)}</p><div class="meta">${fmt(s.count)} pieces · ${fmt(s.triangles)} triangles${organs ? ` · ${organs} organ${organs > 1 ? 's' : ''}` : ''}</div></a>`;
}).join('');
