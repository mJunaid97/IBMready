...
import { renderHeader, renderFooter, TYPES, typeLink, entityLink, link, loadClinical, esc, fmt, SITE, PRERENDERED } from './site.js?v=1.1.2';
import { applyMeta, webPageNode } from './seo.js?v=1.1.2';
...
if (!PRERENDERED) {
  ...
  document.getElementById('types').innerHTML = ['symptoms', 'conditions', 'tests', 'imaging', 'procedures', 'medications', 'drug-classes'].map(card).join('');
  document.getElementById('everyday').innerHTML = card('first-aid') + card('health') + `<a class="card" href="${link.search()}"><div class="icon">🔍</div><h3>Search everything</h3><p>One search across structures, organs, systems, terms and every clinical topic, with results that link straight into the 3D atlas.</p></a>`;
