...
import { renderHeader, renderFooter, TYPES, typeLink, entityLink, link, loadClinical, esc, fmt, SITE, PRERENDERED } from './site.js?v=1.3.0';
import { applyMeta, webPageNode } from './seo.js?v=1.3.0';
...
if (!PRERENDERED) {
  ...
  document.getElementById('hubs').innerHTML = [
    ...
    hub(link.page('study'), 'Study', 'Quizzes, flashcards &amp; viva', 'Identify or locate highlighted structures in 3D, flip through flashcards, and test terminology, conditions, tests and medications.'),
  ].join('');
  ...
  const card = (t) => `<a class="card" href="${typeLink(t)}"><div class="icon">${TYPES[t].icon}</div><h3>${esc(TYPES[t].name)} ${count(t) ? `<span class="badge">${count(t)}</span>` : ''}</h3><p>${esc(TYPES[t].blurb)}</p></a>`;
  ...
  document.getElementById('everyday').innerHTML = card('first-aid') + card('health') + `<a class="card" href="${link.checker()}"><div class="icon">💊</div><h3>Drug Interaction Checker</h3><p>Add two or more medicines, brands or combination products and see what the official sources say about each pair: severity, mechanism, management and monitoring, never a bare "safe".</p></a><a class="card" href="${link.compare()}"><div class="icon">⚖️</div><h3>Comparisons</h3><p>CRP vs ESR, creatinine vs eGFR, ECG vs echocardiogram, CT vs MRI, TSH vs free T4, HbA1c vs glucose, atorvastatin vs rosuvastatin: what each is for and how they differ.</p></a><a class="card" href="${link.search()}"><div class="icon">🔍</div><h3>Search everything</h3><p>One search across structures, organs, systems, terms and every clinical topic, with results that link straight into the 3D atlas.</p></a>`;
  ...
  for (const a of document.querySelectorAll('.hero .actions a, .hero-art, .callout a')) { const h = a.getAttribute('href'); if (h === 'explorer/') a.href = link.explorer(); else if (h === 'anatomy/') a.href = link.page('anatomy'); else if (h === 'conditions/') a.href = typeLink('conditions'); else if (h === 'disclaimer/') a.href = link.page('disclaimer'); else if (h === 'editorial-policy/') a.href = link.page('editorial-policy'); }
}
