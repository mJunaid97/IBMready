...
if (!PRERENDERED) {
  ...
  document.getElementById('hubs').innerHTML = [
    ...
    hub(link.page('anatomy'), 'Anatomy', 'Organs &amp; structures', `${c.organs || 39} anatomy pages: location, structure, blood supply, function and clinical relevance, each with a 3D model.`, `${c.anatomyArticles || 12} full articles`, 'anatomy'),
    hub(link.page('systems'), 'Anatomy', 'Body systems', 'Sixteen system pages with overviews, functions, clinical notes, every modelled structure and the topics that concern them.', '', 'systems'),
    ...
    hub(link.page('medical-terms'), 'Learn', 'Medical terminology', `${c.terms || 149} terms: directions, planes, movements, regions, word parts, disease processes and clinical terms, each linked into the atlas.`, '', 'terms'),
  ...
  document.getElementById('everyday').innerHTML = card('first-aid') + card('health') + `<a class="card" href="${link.interactions()}"><div class="icon">${iconSvg('interactions')}</div><h3>Interaction checker</h3><p>Add medicines, brands, drug classes or combination products and see what the official sources say about each pair: mechanism, management and monitoring, never a bare "safe".</p></a><a class="card" href="${link.compare()}"><div class="icon">${iconSvg('compare')}</div><h3>Comparisons</h3><p>CRP vs ESR, creatinine vs eGFR, ECG vs echocardiogram, CT vs MRI, TSH vs free T4, HbA1c vs glucose, atorvastatin vs rosuvastatin: what each is for and how they differ.</p></a><a class="card" href="${link.search()}"><div class="icon">${iconSvg('search')}</div><h3>Search everything</h3><p>One search across structures, organs, systems, terms and every clinical topic, with results that link straight into the 3D atlas.</p></a>`;
