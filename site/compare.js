 ...
 */
import { renderHeader, renderFooter, loadClinical, loadComparisons, link, entityLink, typeLink, esc, pageId, paths, PRERENDERED, SITE, TYPES, breadcrumbHtml, canonical } from './site.js?v=1.3.0';
import { applyMeta, seoTitle, metaDescription, webPageNode } from './seo.js?v=1.3.0';
import { referencesHtml, editorialHtml } from './entity.js?v=1.3.0';
...
async function renderIndex() {
  ...
  document.getElementById('main').innerHTML = `${breadcrumbHtml(crumbs)}
    ...
    <h2>Other sections</h2><div class="chips">${['tests', 'biomarkers', 'imaging', 'medications', 'drug-classes'].map(t => `<a class="chip" href="${typeLink(t)}">${esc(TYPES[t].icon)} ${esc(TYPES[t].name)}</a>`).join('')}<a class="chip" href="${link.checker()}">💊 Drug Interaction Checker</a><a class="chip" href="${link.tools()}">🧰 Clinical tools</a></div>`;
}
