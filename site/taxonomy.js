 ...
 */
import { renderHeader, renderFooter, loadTestCategories, loadMedicationTaxonomy, loadClinical, loadData, link, entityLink, typeLink, TYPES, esc, pageId, paths, PRERENDERED, SITE, breadcrumbHtml, canonical, dateText } from './site.js';
import { applyMeta, seoTitle, metaDescription, webPageNode } from './seo.js';
import { editorialHtml } from './entity.js';
...
export async function renderTestCategories() {
  ...
  main.innerHTML = `${breadcrumbHtml(crumbs)}
    <div class="section-hero"><div class="eyebrow">🧪 Medical tests · taxonomy</div><h1>Medical test categories</h1>
...
export async function renderTestCategory() {
  ...
  main.innerHTML = `${breadcrumbHtml(crumbs)}
    ...
    <div class="two"><div>
      ...
      <section class="related"><h2>Sections</h2><div class="chips"><a class="chip" href="${typeLink('tests')}">🧪 Medical tests</a><a class="chip" href="${typeLink('biomarkers')}">🧫 Biomarkers</a><a class="chip" href="${typeLink('imaging')}">🩻 Imaging</a><a class="chip" href="${typeLink('procedures')}">🔧 Procedures</a><a class="chip" href="${link.compare()}">⚖️ Comparisons</a></div></section>
...
export async function renderMedicationClasses() {
  ...
  if (!PRERENDERED) {
    ...
    main.innerHTML = `${breadcrumbHtml(crumbs)}
      <div class="section-hero"><div class="eyebrow">💊 Medications · taxonomy</div><h1>Medication classes by therapeutic area</h1>
      <p class="lead">${esc(description)}</p>
      ...
      <p class="actions"><a class="btn btn-primary" href="${link.interactions()}">Medication interaction checker</a><a class="btn" href="${typeLink('drug-classes')}">Drug class pages A–Z</a><a class="btn" href="${typeLink('medications')}">Medicines A–Z</a></p></div>
      ...
      <div id="areas">${mt.areas.map(a => `<section class="tax-area" id="area-${esc(a.id)}"><h2>${esc(a.name)}${a.atc?.length ? ` <span class="mono muted small" title="WHO ATC group">${esc(a.atc.join(', '))}</span>` : ''}</h2><p class="muted">${esc(a.summary)}</p>
